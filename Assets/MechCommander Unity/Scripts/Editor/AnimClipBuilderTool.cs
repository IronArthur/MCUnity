...
namespace MechCommanderUnity
{
    public class AnimClipBuilderTool
    {
        ...
        public static void CreateAnimClip()
        {
            ...
            foreach (var selectedTexture in selectedTextures)
            {
                Sprite[] sprites = GetSpritesFromTexture((Texture2D)selectedTexture);

                string filename = selectedTexture.name + ".anim";
                AnimationClip clip = AnimClipBuilder.CreateClip(sprites, filename);

                string fullClipPath = GetSavePathFromTexture((Texture2D)selectedTexture, filename);
        ...
        static string GetSavePathFromTexture(Texture2D selectedTexture, string filename)
        {
            ...
            string textureDirectory = System.IO.Path.GetDirectoryName(texturePath) + System.IO.Path.DirectorySeparatorChar + "AnimClip" + System.IO.Path.DirectorySeparatorChar;
